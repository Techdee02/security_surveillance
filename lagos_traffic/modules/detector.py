...
from config import (
    ...
    BLUE_RATIO_THRESHOLD, PERSON_CLASS_ID, MOTORCYCLE_CLASS_ID,
    MOTORCYCLE_CONFIDENCE, OKADA_MIN_WIDTH, OKADA_MAX_WIDTH,
    OKADA_PERSON_OVERLAP_THRESHOLD
)
...
class LagosVehicleDetector:
    ...
    def detect(self, frame):
        ...
        Enhanced for better okada (motorcycle) detection in crowded scenes
        ...
        """
        # Run YOLO detection with lower confidence for motorcycles
        # Use lower confidence to catch more motorcycles
        results = self.model(
            frame,
            conf=MOTORCYCLE_CONFIDENCE,  # Lower threshold for motorcycles
            imgsz=INPUT_SIZE,
            device=self.device,
            verbose=False
        )
        ...
        detections = []
        person_boxes = []  # Store person detections to help identify okadas
        motorcycle_candidates = []  # Store potential motorcycles
        ...
        for result in results:
            ...
            for box in boxes:
                ...
                # Store person detections to help identify okada riders
                ...
                # For non-motorcycle vehicles, apply standard confidence threshold
                if class_id != MOTORCYCLE_CLASS_ID and confidence < CONFIDENCE_THRESHOLD:
                    continue
                
                # Get base vehicle type from COCO
                base_type = self.vehicle_classes[class_id]
                
                # Store motorcycle candidates for enhanced processing
                if class_id == MOTORCYCLE_CLASS_ID:
                    motorcycle_candidates.append({
                        'class_id': class_id,
        ...
        # Second pass: Process motorcycles with person association
        for moto in motorcycle_candidates:
            bbox = moto['bbox']
            confidence = moto['confidence']
            
            # Check if this motorcycle has a rider (person nearby/overlapping)
            has_rider = self._check_for_rider(bbox, person_boxes)
            
            # Boost confidence if rider detected
            if has_rider:
                confidence = min(confidence * 1.2, 1.0)  # Boost by 20%
            
            # Classify (check if it's actually a keke)
            vehicle_type = self._classify_lagos_vehicle(
                frame, bbox, "motorcycle", confidence
            )
        ...
        # Third pass: Look for okadas that might be missed
        # Check person detections that might be okada riders
        additional_okadas = self._find_hidden_okadas(frame, person_boxes, detections)
        detections.extend(additional_okadas)
        
        return detections
    
    def _check_for_rider(self, moto_bbox, person_boxes):
        """Check if a motorcycle has a rider (person) nearby"""
        ...
        for person in person_boxes:
            ...
            # Check if person is above or overlapping the motorcycle
            # Riders are typically positioned above the motorcycle
            horizontal_overlap = not (px2 < mx1 or px1 > mx2)
            person_above_or_on = py2 >= my1 - 50  # Person's bottom near or above motorcycle top
            
            if horizontal_overlap and person_above_or_on:
                return True
        
        return False
    
    def _find_hidden_okadas(self, frame, person_boxes, existing_detections):
        """
        Find okadas that might be missed by looking at person detections
        DISABLED: This was causing too many false positives (detecting people as okadas)
        The standard YOLO motorcycle detection is sufficient for okada detection
        """
        # Return empty list - disabled due to false positives
        return []
